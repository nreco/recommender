...
sealed class FileItemItemSimilarityIterable : Iterable<GenericItemSimilarity.ItemItemSimilarity> {

  private File similaritiesFile;

  FileItemItemSimilarityIterable(File similaritiesFile) {
    this.similaritiesFile = similaritiesFile;
  }

  public IEnumerable<GenericItemSimilarity.ItemItemSimilarity> iterator() {
    try {
      return new FileItemItemSimilarityIterator(similaritiesFile);
    } catch (IOException ioe) {
      throw new InvalidOperationException("Can't read " + similaritiesFile, ioe);
    }
  }
